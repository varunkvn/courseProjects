...
public class EnlistFacultySubject extends MESFrame implements ActionListener{

    MainFrame parent;
    FacultyChecking fChecking;
    ...
    public EnlistFacultySubject(MainFrame main,FacultyChecking paramFCheck){
        super(600,370,"", new String[] {"Ok","Close"});
        parent = main;
        fChecking=paramFCheck;
        init();
    ...
    return;
       ...
       if(intchk==JOptionPane.YES_OPTION)
       {
            ...
            String sql = "INSERT INTO facultyloading_data(EmployeeNo,ClassNo,SubjectCode,Title,LecUnit,LabUnit,Days,TSlot,Room)VALUES('"+fChecking.strID+"','"+number+"','"+question+"','"+answer+"','"+lecture+"','"+laboratory+"','"+day+"','"+time+"','"+room+"')";
            int update = parent.database.executeUpdate(sql);
            if(update ==0)																																																								
            {
                Hashtable h = new Hashtable();
                h.put("EmployeeNo",fChecking.strID);
                ...
                h.put("TSlot",time);
                h.put("Room",room);
                parent.getCollection("facultyloading_data").add(h);
            }
             else{
                return;
            }
