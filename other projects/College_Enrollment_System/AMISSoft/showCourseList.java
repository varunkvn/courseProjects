...
public class showCourseList extends MESFrame implements ActionListener{

    MainFrame parent;
    MESTable table;
    JTextField keyMaster;
    public showCourseList(MainFrame main)
    {
        super(700,300,"",new String[] {"Print","Add Subjects","Class Subjects","Close"}  );
        setTitle("Curriculum Subjects");
        parent = main;
        init();
        setVisible(true);
      } 
    void init()
    {
        ...
        table = new MESTable("",new String[] {"CourseCode","Title","TotalUnits","CurrID"});
        table.setColumnSize(new int[] {80,350,120,120});
        MAINPANEL.add(table);
    ...
    void fillTable()
    {
        DataCollection col = parent.getCollection("Courses");
        table.getTableModel().setRowCount(0);
        
        for(int i = 0; i < col.size(); i++){
            Hashtable data  = (Hashtable)col.get(i);
            Object[] row = { new JLabel(data.get("CourseCode")+""),
                             new JLabel(data.get("Title")+""),
                             new JLabel(data.get("TotalUnits")+""),
                             new JLabel(data.get("CurrID")+""),
