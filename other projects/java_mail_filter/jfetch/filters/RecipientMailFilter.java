...
/**
 * Restrict the download of mail through a recipient based filter.
 * Recipient ids are stored in a properties file.
 *
 * Configuration is of the form <br>
 * &lt; filter class="..." delete="true" 
 * blocklist="../spool/rcpt.list" &gt; <br>
 * &lt; /filter &gt;
 *
 * 
 */
public class RecipientMailFilter implements MailFilter, Configurable, Initializable {
    ...
    public void init() {
        logger = LogKit.getLoggerFor("rcptfilter");
        loadProps();
        if(mdaid != null)
    ...
    /**
     * Check for the recipient of the message in the cache
     *
     * @param message a <code>Message</code> value
     * @return a <code>boolean</code> value
     */
    public boolean process( Message message ) {
		RecipientStringTerm toTerm, ccTerm;
		boolean found = false;
		Enumeration propsEnum = props.propertyNames();

        while( propsEnum.hasMoreElements() ) {
            String addr = (String)propsEnum.nextElement();
            toTerm = new RecipientStringTerm( Message.RecipientType.TO, addr );
            ccTerm = new RecipientStringTerm( Message.RecipientType.CC, addr );
            if( toTerm.match( message ) || ccTerm.match( message ) ) {
                found = true;
                break;
            }
        }
        ...
        if( found ) {
            ...
            if(delete) {
                logger.info("Message recipient found in list; deleting, " + MessageUtil.toString(message));
                try {
                ...
                logger.info("Message recipient found in list, skipping, " + MessageUtil.toString(message));
            }
            return false;
        } else {
            return true;
